...
def build_request(
    scenario: scenario_mod.Scenario, rng: random.Random, pool: scenario_mod.Pool
) -> tuple[curlrun.Request, str]:
    domain = pool.pick(rng)
    proto = "h3" if rng.random() < scenario.quic_share else "h2"
    request = curlrun.Request(domain=domain, path=scenario.object_path, proto=proto)
...
async def run_worker(
    ...
    while True:
        ...
        argv = curlrun.build_argv(scenario, request, cacert=str(scenario.run.cacert))
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    if duration is None and not args.requests:
        raise SystemExit("runner: podaj --duration ali --requests")
    deadline = None if args.requests else time.monotonic() + duration

    try:
        groups = scenario_mod.parse_groups(args.groups)
        pool = scenario_mod.build_pool(scenario, groups)
