...
TOPO="${1:?uporaba: start.sh <postavitev> [--no-content-block] [--web] [--lazy]}"
...
for arg in "$@"; do
	case "$arg" in
	--no-content-block) CONTENT_BLOCK=0 ;;
	--web) WEB=1 ;;
	--lazy) LAZY=1 ;;
...
case "$TOPO" in
C0) HAS_SWITCH=0 HAS_SERVER=1 HAS_BROWSER=0 HAS_PROXY=0 ;;
...
$SUDO clab deploy -t "$TOPO_FILE" --reconfigure >/dev/null
...
if [ "$HAS_SWITCH" = 1 ]; then
	...
	if tail -n 20 "$OUT/switch.log" 2>/dev/null | grep -q "Skipping module"; then
		echo "start.sh: bmv2 ni nalozil quic_sni.so, ker slika ni povezana z -rdynamic;" >&2
		echo "  pozeni docker rmi p4-switch:latest bmv2-perf:1.15.5-modules && ./orodja/build.sh" >&2
		exit 1
	fi
...
sys.path.insert(0, str(Path("okolje/client").resolve()))
from runner import scenario as scenario_mod

scenario = scenario_mod.load("okolje/experiment.yml", testset="okolje/server/testset")
...
if [ "$HAS_PROXY" = 0 ]; then
	phase "brez posrednika, promet gre naravnost"
fi

ADDONS=(-s /opt/traffic/proxy/proxy_stats.py)
if [ "$HAS_PROXY" = 1 ] && [ "$CONTENT_BLOCK" = 1 ]; then
...
if [ "$HAS_BROWSER" = 1 ]; then
	phase "brskalnik: ./orodja/browse.sh $TOPO [chromium|firefox] <url>"
fi
