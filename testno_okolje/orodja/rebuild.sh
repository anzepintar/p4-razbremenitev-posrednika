...
--clean | --clean-all)
	echo "== ciscenje =="
	for topo in A0 B0 B1; do
		$SUDO clab destroy -t "$topo.clab.yml" --cleanup >/dev/null 2>&1 || true
	done
	rm -rf okolje/out okolje/server/testset okolje/switch/build \
		okolje/lists/*.txt okolje/lists/assignment.json \
		okolje/server/sites.caddy okolje/pki/trust.pem
...
if [ -d okolje/server/testset/testni ]; then
	echo "  ze zgrajen, preskocim"
else
	./orodja/build_testset.py
fi
...
echo "== slike docker =="
MITM_SRC="$MITM_SRC" ./orodja/build.sh
