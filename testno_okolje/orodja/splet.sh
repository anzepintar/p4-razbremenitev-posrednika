...
# Kontrolna zahteva ob zagonu: privzeto stran, ki jo pregled tako ali tako obisce in
# ki streze h3, da postavitev ni odvisna od lastne domene.
...
# Siva pot mora ostati siva: ce bi bila katera od domen nabora na laboratorijskih
# seznamih iz okolje/lists, bi jo stikalo obravnavalo po politiki poglavja 5.
...
sweep() {
	...
	for client in $SWEEP_CLIENTS; do
		for proto in $SWEEP_PROTOS; do
			# Stevci se berejo na blok, zato je iz njih razvidno, koliko imen je
			# stikalo prebralo iz pravih sporocil ClientHello tega odjemalca.
...
# Dnevnik sej se dopisuje, zato ga pred B1 pobrisemo; sicer bi porocilo gledalo se
# seje prejsnjega zagona.
rm -f "$OUT/proxy_flows.jsonl"
# Vsebinski filter je izklopljen: pravilo ~bs prisili posrednika, da vsak odgovor
# v celoti shrani v pomnilnik, oznake x-block-me pa na pravem spletu ni nikjer.
bring_up B1 --no-content-block
sweep B1

# Dnevnik sej posrednika je neodvisen vir: pove, ali je posrednik sejo sploh videl.
