...
mkdir -p "$COMMON/out"
rm -f "$COMMON/out/bypass.jsonl" "$COMMON/out/mitm.log" "$COMMON/out/controller.log"
SUDO="$SUDO" "$COMMON/start.sh" "$TOPO" >/dev/null && ok "topologija stoji, krmilnik in posrednik teceta" || {
	fail "postavitev se ni zagnala"
	exit 1
}
...
echo
echo "== promet skozi posrednik z addonom =="
