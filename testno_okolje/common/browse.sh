...
TOPO="${1:?uporaba: browse.sh <A1|B1> [chromium|firefox] [url]}"
BROWSER="${2:-chromium}"
URL="${3:-${PROBE_URL:-https://quic.anzepintar.com/}}"

case "$TOPO" in
A1 | B1) ;;
*) echo "browse.sh: brskalnik je le v A1 in B1, ne v '$TOPO'" >&2; exit 2 ;;
esac

NODE="clab-$TOPO-client"
WEB_PORT="${VNC_WEB_PORT:-6080}"
docker exec "$NODE" true 2>/dev/null || {
	echo "browse.sh: $NODE ne tece; pozeni ./common/start.sh $TOPO" >&2
	exit 1
}
...
if ! docker exec "$NODE" ss -lnt 2>/dev/null | grep -q ":$WEB_PORT"; then
	...
	docker exec "$NODE" ss -lnt 2>/dev/null | grep -q ":$WEB_PORT" || {
		echo "browse.sh: novnc se ni zagnal, glej common/out/vnc.log" >&2
		exit 1
	}
fi
